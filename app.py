...
def create_app(test_config=None):
    ...
    app = Flask(__name__)
    setup_db(app)
    # db_drop_and_create_all() # uncomment this if you want to start
    # a new database on app refresh
    ...
    def get_error_message(error, default_message):
        '''Returns default error message or custom
        error message (if not applicable)

        *Input:
            * <error> system generated error message
            which contains a message description
            * <string> default message to be used as
            error message if Error has no specific message
        *Output:
            * <string> specific error message or default
            error message(if no specific message is given)
    ...
    #  API Endpoints
    #  ----------------------------------------------------------------
    #  NOTE:  For explanation of each endpoint, please have look at
    #         the README.md file.
    #         DOC Strings only contain short description and list of
    #         test classes
    # ----------------------------------------------------------------------------#
    ...
    def edit_actors(payload, actor_id):
        ...
        if not actor_id:
            abort(400,
                  {'message':
                   'Actor id URL parameter is required'
                   ', please add an actor id to the request URL.'})
        ...
        if not actor_to_update:
            abort(404,
                  {'message':
                   'Actor with id {} does not exist.'.format(actor_id)})

        # Get name and age value from request body
        # If not given, set the same existing field values,
        # hence no update will happen
    ...
    def insert_movies(payload):
        ...
        if not release_date:
            abort(422,
                  {'message': 'No "release_date" provided'
    ...
    def edit_movies(payload, movie_id):
        ...
        if not movie_id:
            abort(400,
                  {'message':
                   'Movie id URL parameter is required'
                   ', please add a movie id to the request URL.'})
    ...
    def delete_movies(payload, movie_id):
        ...
        if not movie_id:
            abort(400,
                  {'message': 'Movie id URL parameter is required'
                   ', please add movie id to the request URL.'})
